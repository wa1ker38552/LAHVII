                ...
                if transcript:
                    if incoming_command:
                        if 'applications' in transcript.lower() and 'open' in transcript.lower():
                            ...
                            target = f'+1{transcript.lower().split("text")[-1]}'
                            speak(f'What message do you want to send to {target.replace("+1", "")}?')
                            time.sleep(5)
                            ...
                            try:
                                send_text(target, text['alternative'][0]['transcript'])
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            print('Connection closed unexpectedly! (GPT took longer than 20 seconds to respond)')
            running = False
